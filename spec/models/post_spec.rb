...
RSpec.describe Post do
  ...
  describe 'validations' do
    ...
    it { is_expected.to define_enum_for(:status).with_values(%i[draft published archived]) }
  end
  ...
  describe 'scopes' do
    ...
    describe '.by_user' do
      ...
      it 'includes posts by the given user' do
        expect(described_class.by_user(user)).to include(published_post, draft_post)
      end

      it 'excludes posts by other users' do
        expect(described_class.by_user(user)).not_to include(other_post)
      end
    end
  end
  ...
  describe '#can_be_edited_by?' do
    ...
    it 'returns true for the post owner' do
      expect(post).to be_can_be_edited_by(user)
    end

    it 'returns false for other users' do
      expect(post).not_to be_can_be_edited_by(other_user)
    end
  end
end
